...
**API (SaaS Fase 1) — `api/main.py` + `authcore.py`.** `authcore.py` (root, no Streamlit) holds the shared auth: DB, scrypt hashing, revocable session tokens, codes, roles, `ENTITLEMENTS`; `ui/auth.py` is Streamlit glue over it, and the API uses the same tokens as Bearer credentials — a login works on both surfaces. FastAPI endpoints under `/v1`: auth (register/login/logout/me), `analyze` (multipart uploads → background job via in-process ThreadPool + `jobs` table; plan `max_tracks` enforced server-side), `jobs/{id}` + `/result`, `playlist` (plateau gated), `export/{fmt}` (csv/m3u free; rekordbox/serato/traktor Pro). **Content-hash cache** (`feature_cache`, keyed sha256+duration) skips librosa on repeats; **job audio is deleted in a `finally`** — only feature JSON survives (rhythm_vector stored as list, re-hydrated to numpy for scoring). Run: `uvicorn api.main:app --port 8000`; e2e suite: `python tests/test_api.py`. Pin caution: fastapi must be ≥0.139 so starlette stays ≥0.45 for Streamlit.

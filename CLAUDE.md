...
- `analyzer.py` = the tool, **no sidebar**. It walks three guided states — setup panel (no tracks, with a 1-2-3 steps strip) → analyzing (progress) → results — controlled by `st.session_state.tracks is None`. Setup offers **two sources**: a local folder, or file **uploads** (stashed into a per-session temp dir by `_stash_uploads`, then analyzed through the same folder path; uploads take priority). The app is **cloud-deployable** (Streamlit Community Cloud): Tkinter is imported lazily and the Browse button hides when unavailable (`folder_picker_available`), and `packages.txt` provides ffmpeg/libsndfile on the host. Results are split into **six modules** behind a `st.segmented_control` switcher styled as underline nav tabs (`MODULES`: Set builder / Inspector / Discover / Insights / Data / Export, each with a `MODULE_DESCRIPTIONS` line); only the active module renders. Global above the switcher: the shared **app header** (`components.render_app_header`: brand + breadcrumb + Home), the **control bar** (labeled "Set controls": start-track, energy-curve segmented control, exclude/weights popovers, "New folder"), and status chips with the 📖 Guide popover. The Inspector module (`render_track_inspector`) previews a track, shows its top-3 compatible next tracks from the transition matrix, and offers inline key/BPM correction via `state.override_track` (which recomputes Camelot and calls `invalidate_order()` to force a re-score).

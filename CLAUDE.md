...
**API (SaaS Fase 1) — `api/main.py` + `authcore.py`.** `authcore.py` (root, no Streamlit) holds the shared auth: DB, scrypt hashing, revocable session tokens, codes, roles, `ENTITLEMENTS`; `ui/auth.py` is Streamlit glue over it, and the API uses the same tokens as Bearer credentials — a login works on both surfaces. **Dual auth** (`current_user`): a Bearer with two dots is a Supabase JWT → `api/supabase_auth.py`; anything else is an authcore opaque token. `supabase_auth` verifies **offline via the project's public JWKS** (ES256 — what current stacks sign; HS256 fallback via `SUPABASE_JWT_SECRET` for legacy projects), then reads the plan from `public.profiles` via PostgREST with the service key (60 s cache; **missing profile row = deleted account → 401**, near-real-time revocation despite stateless JWTs). Configured by `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` env; unset = Supabase tokens politely rejected. FastAPI endpoints under `/v1`: auth (register/login/logout/me), `analyze` (multipart uploads → background job via in-process ThreadPool + `jobs` table; plan `max_tracks` enforced server-side), `jobs/{id}` + `/result`, `playlist` (plateau gated), `export/{fmt}` (csv/m3u free; rekordbox/serato/traktor Pro). **Content-hash cache** (`feature_cache`, keyed sha256+duration) skips librosa on repeats; **job audio is deleted in a `finally`** — only feature JSON survives (rhythm_vector stored as list, re-hydrated to numpy for scoring). Run: `uvicorn api.main:app --port 8000`; suites: `python tests/test_api.py`, `tests/test_api_hardening.py`, `tests/test_api_supabase.py` (integration vs the local Supabase stack; SKIPs cleanly when it's down). Pin caution: fastapi must be ≥0.139 so starlette stays ≥0.45 for Streamlit.

**Web (SaaS Fase 2) — `web/` (Next 16 + React 19 + Tailwind v4).** Read `web/AGENTS.md` first: this Next has breaking renames (middleware → `proxy.ts`) — check `node_modules/next/dist/docs/` before writing Next-specific code. Supabase auth via `@supabase/ssr` (`lib/supabase/client|server.ts`, both return `null` when env unset), route guards in `proxy.ts`, design tokens in `app/globals.css` (`@theme`: bg/ink/mint/lavender/line/surface + Outfit/Inter/DM Mono). `lib/entitlements.ts` mirrors `authcore.ENTITLEMENTS` — keep them in sync. The **Analyze module** (`app/app/analyze/page.tsx` server gate + `components/analyze/AnalyzeClient.tsx`) calls the FastAPI directly from the browser (`lib/api.ts`, `NEXT_PUBLIC_API_URL`) with the Supabase access token: drop/browse files → `POST /v1/analyze` → poll `jobs/{id}` (progress bar) → `POST /v1/playlist` → table with Camelot chips (same hue mapping as `CamelotWheel.tsx`) + energy SVG + export downloads (free formats live, Pro ones locked to `/#pricing` for free users). Supabase migrations: **every new table needs explicit grants** (see `0002_table_grants.sql`) — the stack does NOT auto-grant CRUD to anon/authenticated/service_role, and RLS policies are useless without table privileges (service_role bypasses RLS but not grants). Local stack: `npx supabase start`; migrations: `npx supabase migration up`.

**Accounts + roles (`ui/auth.py`).** Self-contained for the Streamlit phase: SQLite at `~/.keyflow/keyflow.db`, scrypt-hashed passwords, session in `st.session_state["auth_user"]`, **persisted across refreshes** via a `kf_session` cookie: tokens live hashed in a `sessions` table (30-day TTL), the cookie is read natively with `st.context.cookies` and written via a `components.html` script — **deferred to the run after login** (`_cookie_set` flag), because writing it in the same run as `st.rerun()` tears the iframe down before the script executes. Password reset + email verification use one-time hashed 6-digit codes (15 min) sent through `ui/mailer.py` (stdlib SMTP, `smtp_*` secrets or `TA_SMTP_*` env; degrades gracefully when unconfigured). `set_password` revokes every session. Roles `free/pro/lifetime`; **`auth.ENTITLEMENTS` is the single source of plan limits** (free: 50 tracks/analysis, no plateau curve, no Discover, no DJ exports). The analyzer gates on login (`_render_auth_gate`); enforcement points: `run_analysis_flow` (track cap via `analyze_folder(limit=)`), `render_control_bar` (plateau), `_module_discover` and `premium.is_premium()` (now role-based). Upgrades: the **Account** module links a Gumroad key via `auth.link_license` — checked against the membership product (→ pro) and `gumroad_lifetime_product_id` (→ lifetime); Pro is re-verified weekly on login and only an authoritative `invalid` downgrades.
